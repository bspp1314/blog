...
对数据的操作其实就只有读或写两种操作，数据库在实现锁时，也会对这两种操作使用不同的锁；InnoDB 实现了标准的行级锁，也就是共享锁（Shared Lock）排他锁（Exclusive Lock）；共享锁和排他锁的作用其实非常好理解：

- 共享锁  （Shared lock） 允许事务对一条行数据进行读取；
- **排他锁（Exclusive Lock）**：允许事务对一条行数据进行删除或更新；
...
一致性的非锁定读（consistent nonlocking read） 是指InnoDB存储引擎通过多版本控制(multi Versioning)的方式来读取当前执行时间数据度中行的数据。如果读取的行正则执行DELETE或UPDATE操作，这时读取操作不会因此去等待行上锁的释放。相反底，InnoDB存储引擎会去读取行的一个快照数据。
...
![image-20210328162650348](image-20210328162650348.png)
...
![image-20210328162926506](image-20210328162926506.png)
...
![image-20210328150600763](image-20210328150600763.png)
...
​	解决死锁问题最简单的一种方法就是超时，即当两个事务互相等待时，当一个等待时间超过设置的某一阈值时，其中一个事务进行回滚，另一个等待的事务就能继续进行。在InnoDB存储引擎中，参数 `innodb_lock_wait_timeout` 用来设置超时时间。

超时机制虽然简单，但是其仅通过对事务进行回滚的方式处理，或者说其是根据FIFO的顺序选择回滚对象。当若超时的事务比较多，占用了较多的undo log,这时采用FIFO的方式，就不合适。



## Wait-for graph 

Wait-for graph 是一种更主动的死锁检测机制。InnoDB也采用了这种方式。wait-for graph 要求数据库保存一下两种信息
...
![image-20210328154538882](image-20210328154538882.png)



如上Transaction Wait Lists 我们可以得到 t1、t2、t3、t4 的 有向图

![image-20210328154957703](image-20210328154957703.png) 



通过该图可以发发现存在 （t1,t2换），因此存在死锁。通过上述图和描述可以发现wait-for graph 是一种主动监测死锁的机制。若存在死锁，通常来说InnoDB存储引擎寻找回滚undo 量最小的事务。	
